...
type
  TFunc = Class
   ...
   function XORLOgic(cmpndfunktion : array of extended):Extended;
   ...
   function VarAssignString(funks : string; vle : TValueListEditor):String;

   function TrueFalse(funks : string) : byte;

   function IFState(funks : string):byte;
...
function Tfunc.VarAssignString(funks : string; vle : TValueListEditor):String;
var
 i : integer;
 change : integer;
 nme, strval : string;
...
begin
 ...
 if change = -1
  then begin
   ...
   vle.InsertRow(nme, strval, false);
  end else
   vle.Cells[1, change] := strval;
...
function TFunc.XORLOgic(cmpndfunktion : array of extended):Extended;
var
 y : integer;
begin
 Result := 0;
   ...
   begin
    ...
    for y := 2 to length(cmpndFunktion)-1 do
     if (Result <> 0)and(trunc(cmpndFunktion[y]) <> 0)
      then
       Result := ( trunc(Result) xor trunc(cmpndFunktion[y]) );
   end;
end;
...
function TFunc.DecToRom(Decimal: Longint): string;
 const
  Numbers: array[1..22] of Integer =                //numbers used
   (1, 4, 5, 9, 10, 40, 50, 90, 100, 400, 500, 900, 1000, 4000,
    5000, 10000, 40000, 50000, 100000, 400000, 500000, 1000000);
  Romans: array[1..22] of string =                 //romans to be converted too
   ('I', 'IV', 'V', 'IX', 'X', 'XL', 'L', 'XC', 'C', 'CD', 'D', 'CM', 'M', 'M(V)',
    '(V)', '(X)','(X)(L)','(L)', '(C)','(C)(D)','(D)', '(M)');
 var
  i: Integer;                                   //used to begin loop
 begin
  Result := '';                                  //used to output data
  for i  := 22 downto 1 do                        //for 22 to 1 do this
   while (Decimal >= Numbers[i]) do    //as long as decimal is smaller than No.[i]
    begin
     Decimal := Decimal - Numbers[i];        //done for Result
     Result  := Result  + Romans[i];           //continue to add array to equals as long as
    end;
 end;
...
function TFunc.IFState(funks : string): byte;
var
 Logic : TLogic;
begin
 Logic := TLogic.Create;
...
begin
 ...
 if Length(cmpndfunktion) > 2
  then
   if (cmpndfunktion[1] = 1) and (cmpndfunktion[2] = 1)
    then
     Result := 1
    else
     Result := 0;
...
begin
 ...
 if Length(cmpndfunktion) > 2
  then
   if (cmpndfunktion[1] = 1) or (cmpndfunktion[2] = 1)
    then
     Result := 1
    else
     Result := 0;
